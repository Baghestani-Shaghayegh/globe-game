...
type Question = { geoName: string; displayName: string; clues: string[] };
...
/** "Famous for": a clue is shown and the player names the country. */
export default function FamousGame({ mode, limitMs, ruleset }: Props) {
  ...
  /** How many of this country's clues have been shown so far. */
  const [cluesShown, setCluesShown] = useState(1);
  const [secondsLeft, setSecondsLeft] = useState(BLITZ_SECONDS);

  const round = useRound(recordKey("famous", mode.id, limitMs, ruleset), limitMs);
  ...
  useEffect(() => {
    ...
    fetch("/data/world.geojson")
      ...
      .then((data: { features: { properties: { name: string } }[] }) => {
        if (cancelled) return;
        // Only countries someone has written a clue for.
        const playable = data.features
          ...
          .map((meta) => ({
            ...
            clues: cluesFor(meta.geoName),
          }))
          .filter((q) => q.clues.length > 0);
  ...
  const handleAnotherClue = () => {
    if (!current || revealed || cluesShown >= current.clues.length) return;
    spendHint("continent");
    ...
    <div className="relative min-h-screen bg-[#07111c]">
      ...
      {current && !summary && (
        <main className="flex min-h-screen flex-col items-center justify-center px-5 py-24">
          <div
            className={`w-full max-w-md rounded-xl border border-white/10 bg-white/[0.04] px-6 py-7 text-center backdrop-blur-sm ${
          ...
          >
            ...
            <ul className="mt-3 flex flex-col gap-2">
              {current.clues.slice(0, cluesShown).map((clue) => (
                <li key={clue} className="text-lg text-zinc-100 sm:text-xl">
                  {clue}
                </li>
              ))}
            </ul>
            ...
            {revealed ? (
              ...
              <>
                <div className="flex items-baseline justify-between gap-3 pb-1.5">
                  ...
                  {ruleset === "blitz" && (
                    <span
                      ...
                      aria-label="Seconds left on this clue"
                    >
                      {secondsLeft}s
                    </span>
                  )}
                ...
                <div className="mt-3 flex items-center gap-3 text-xs">
                  {cluesShown < current.clues.length ? (
                    <button
                    ...
                    <span className="px-2 py-1 text-zinc-600">No more clues</span>
                  )}
                  <button
                    onClick={handlePass}
